...
class QuaternionVisitor:  public py::def_visitor<QuaternionVisitor<QuaternionT> >{
	...
	static QuaternionT* fromRPY(const Scalar& roll, const Scalar& pitch, const Scalar& yaw){ 
		...
		*q = AngleAxisT(yaw, CompatVec3::UnitZ()) 
		* AngleAxisT(pitch, CompatVec3::UnitY()) 
		* AngleAxisT(roll, CompatVec3::UnitX()); 
		return q; 
	}
	...
	static CompatVec3 toRPY(const QuaternionT& self){ 
		CompatVec3 ypr = self.toRotationMatrix().eulerAngles(2,1,0); 
